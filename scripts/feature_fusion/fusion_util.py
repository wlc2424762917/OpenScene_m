...
def extract_openseg_img_feature(img_dir, openseg_model, text_emb, img_size=None, regional_pool=True):
    ...
    feat_2d = torch.from_numpy(feat_2d).permute(2, 0, 1)  # [768, H, W]
...
def save_fused_feature(feat_bank, point_ids, n_points, out_dir, scene_id, args):
    ...
    for n in range(args.num_rand_file_per_scene):
        ...
        rand_ind = np.random.choice(range(n_points), n_points_cur, replace=False)  # randomly select n_split_points points
...
class PointCloudToImageMapper(object):
    ...
    def compute_mapping(self, camera_to_world, coords, depth=None, intrinsic=None):
        ...
        :return: mapping, N x 3 format, (H,W,mask); the corresponding pixel coordinates of each point
        """
        if self.intrinsics is not None:  # global intrinsics
            intrinsic = self.intrinsics

        mapping = np.zeros((3, coords.shape[0]), dtype=int)  # [3, N]
        coords_new = np.concatenate([coords, np.ones([coords.shape[0], 1])], axis=1).T  # [4, N]
        assert coords_new.shape[0] == 4, "[!] Shape error"

        world_to_camera = np.linalg.inv(camera_to_world)  # inverse of extrinsics [4, 4]
        p = np.matmul(world_to_camera, coords_new)  # [4, N]
        # project the points to the image plane, p[2] is the depth
        p[0] = (p[0] * intrinsic[0][0]) / p[2] + intrinsic[0][2]  # [N]
        p[1] = (p[1] * intrinsic[1][1]) / p[2] + intrinsic[1][2]  # [N]
        pi = np.round(p).astype(int)  # simply round the projected coordinates
        # check if the projected coordinates are inside the image
        ...
        # check if the projected coordinates are visible
        if depth is not None:
            depth_cur = depth[pi[1][inside_mask], pi[0][inside_mask]]
            # check if the depth difference is within the threshold, if not, it is occluded
        ...
        # why pi[1] and pi[0] are swapped?
        # because the image is in H x W format, but the point cloud is in W x H x D format
        mapping[0][inside_mask] = pi[1][inside_mask]
        mapping[1][inside_mask] = pi[0][inside_mask]
        mapping[2][inside_mask] = 1  # mask 1 means the point is visible

        return mapping.T  # [3, N] --> [N, 3]
