...
class FusedFeatureLoader(Point3DLoader):
    ...
    def __init__(self,
        ...
        if 'nuscenes' in self.dataset_name: # only one file for each scene
            ...
            self.list_occur = []  # for training sets, ScanNet and Matterport has 5 each, nuscene 1
            ...
            ind = np.where(np.array(self.list_occur) != 0)[0]  # get the indices of non-zero elements
            if np.any(np.array(self.list_occur)==0):  # if there are some scenes with no features
                # only keep the scenes with features
        ...
        if len(self.data_paths) == 0:
            print(self.datapath_feat)
            raise Exception('0 file is loaded in the feature loader.')

    def __getitem__(self, index_long):
        """
        index_long: index of the point cloud in the dataset
        """
        index = index_long % len(self.data_paths)  # for training, we loop over the dataset
        if self.use_shm:  # attach to the shared memory
            pass
            ...
            labels_in[labels_in == -100] = 255  # set invalid labels to 255
            ...
            if np.isscalar(feats_in) and feats_in == 0:
                ...
                feats_in = (feats_in + 1.) * 127.5  # scale color to 0-255
        ...
        if 'nuscenes' not in self.dataset_name:
            # randomly select one of the multiple views of the same scene
        ...
        if self.eval_all:
            ...
            # coords (N, 4), feats (N, 3), labels (N), feat_3d (N, 3), mask (N), inds_reconstruct (N)
